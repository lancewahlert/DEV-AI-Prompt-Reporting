...
def main(templateFilename = './src/json/TemplatesComplex.json', useLocalModel = False, encode = False, truncateLoad = False, loadSnowflake = False, runQueries = False, saveStageFiles = False, production = False):
    ...
    for template in templates:
        ...
        while not done: # for each parameter combination
            ...
            if(loadSnowflake):
                ...
                stageQ.write('%s|%s|%s|%s|%s|%s|%s|%s\n' % (newDesc, batch, 'store-spotlight', newQuery, enc_bin, enc_json, query_cache, query_cache_ts))
    ...
    if loadSnowflake:
        ...
        stageQ.close()
        schema = 'PC'
        tableQuery = 'OPTIONS_QUERY_COMPLEX'
        if production:
            tableQuery = 'OPTIONS_QUERY_COMPLEX'
        stageQuery = '@PC.PC_QUERY_OPTION_STAGE'

        print('Using table %s' % (schema+'.'+tableQuery))
